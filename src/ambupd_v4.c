...
extern void ambupd_write_epoch(const rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (iobs = 0; iobs < n; iobs++) {
        ...
        {
            double L_corr[NFREQ]={0}, P_corr[NFREQ]={0}, frq_corr[NFREQ]={0};
            int frq_idxs[NFREQ]={1,2,3};

            getcorrobs(&rtk->opt, obs+iobs, nav, frq_idxs, NULL, NULL,
                       rtk->ssat[sat-1].phw, L_corr, P_corr, NULL, NULL,
                       frq_corr, NULL, NULL);
