                     ...
                     double bl, double dt, int f, const prcopt_t *opt, const obsd_t *obs)
{
    ...
    if (opt->rcvstds&&obs->L[frq]!='\0'&&obs->P[frq]!='\0') {
       /* include err ratio and measurement std (P or L) from receiver */
       if (code) fact=opt->eratio[frq]*obs->P[frq];
       else fact=obs->L[frq];
    ...
    if(opt->ionoopt == IONOOPT_IFLC) return SQR(3.0);
    else return fact * 2.0 * ( SQR(a) + SQR(b / sinel) + SQR(c) ) + SQR(d);
}
/* baseline length -----------------------------------------------------------*/
...
static int ddidx(rtk_t *rtk, int *ix, int gps, int glo)
{
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa,int gps,int glo)
{
    prcopt_t *opt=&rtk->opt;
    ...
    ix=imat(nx,2);
    if ((nb=ddidx(rtk,ix,gps,glo))<(4)) {  /* nb is sat pairs */
...
static int manage_amb_LAMBDA(rtk_t *rtk, double *bias, double *xa, const int *sat, int nf, int ns) 
{
    ...
    if (rtk->sol.prev_ratio2<rtk->sol.thres&&rtk->nb_ar>=1) {
    ...
    nb=resamb_LAMBDA(rtk,bias,xa,gps1,glo1);
