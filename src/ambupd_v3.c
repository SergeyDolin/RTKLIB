...
/* Melbourne-Wubbena (MW/HMW) wide-lane ambiguity for GPS L1/L5 ----------------
 * MW in cycles:
 *   N_WL = ( (L1*λ1 - L5*λ5) - (P1 - P5) ) / λ_WL
 * where L are carrier phases in cycles, P are pseudoranges in meters.
 * This yields a WL-like ambiguity (incl. code/phase bias terms) in cycles.
 *
 * We also maintain a simple per-satellite running mean/variance of MW to obtain
 * a reasonable SIG_WL in cycles (needed by GREAT-UPD weighting).
 */
static int mw_stat_init = 0;
static double mw_mean[MAXSAT] = {0};
static double mw_m2  [MAXSAT] = {0};
static int    mw_n   [MAXSAT] = {0};

static int mw_compute_gps_l1_l5(const obsd_t *obs, const nav_t *nav,
                               int kL1, int kL5, double f1, double f5,
                               double *mw_cyc)
{
    if (!obs || !nav || !mw_cyc) return 0;

    /* require both phase and code */
    double L1 = obs->L[kL1], L5 = obs->L[kL5];
    double P1 = obs->P[kL1], P5 = obs->P[kL5];

    if (L1 == 0.0 || L5 == 0.0 || P1 == 0.0 || P5 == 0.0) return 0;

    const double lambda1 = CLIGHT / f1;
    const double lambda5 = CLIGHT / f5;
    const double denom_wl = (f1 - f5);
    if (fabs(denom_wl) < 1.0) return 0;

    const double lambda_wl = CLIGHT / denom_wl; /* m/cycle */
    if (fabs(lambda_wl) < 1e-6) return 0;

    *mw_cyc = ((L1 * lambda1 - L5 * lambda5) - (P1 - P5)) / lambda_wl;
    return finite_d(*mw_cyc);
}

static void mw_update_stat(int sat, double mw_cyc)
{
    /* sat is 1..MAXSAT */
    int i = sat - 1;
    if (i < 0 || i >= MAXSAT) return;

    if (!mw_stat_init) {
        /* arrays are already zeroed, this flag is just a reminder */
        mw_stat_init = 1;
    }

    /* Welford online update for mean/variance */
    mw_n[i] += 1;
    double delta  = mw_cyc - mw_mean[i];
    mw_mean[i] += delta / (double)mw_n[i];
    double delta2 = mw_cyc - mw_mean[i];
    mw_m2[i]  += delta * delta2;
}

static int mw_get_stat(int sat, double *mean_cyc, double *sigma_cyc)
{
    int i = sat - 1;
    if (i < 0 || i >= MAXSAT || !mean_cyc || !sigma_cyc) return 0;

    if (mw_n[i] <= 0) return 0;

    *mean_cyc = mw_mean[i];

    /* sample std-dev of MW scatter (not std error of mean) */
    if (mw_n[i] >= 2 && mw_m2[i] > 0.0) {
        *sigma_cyc = sqrt(mw_m2[i] / (double)(mw_n[i] - 1));
    }
    else {
        /* reasonable default at the start of an arc */
        *sigma_cyc = 0.05;
    }
    return 1;
}
...
void ambupd_write_epoch(const rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (sat = 1; sat <= MAXSAT; sat++) {
        ...
        /* WL from Melbourne-Wubbena combination (cycles) + running sigma */
        double mw_raw = 0.0;
        if (!mw_compute_gps_l1_l5(pobs, nav, kL1, kL5, f1, f5, &mw_raw)) continue;

        /* update per-satellite MW statistics to get stable WL and sigma */
        mw_update_stat(sat, mw_raw);

        double N_wl = 0.0, sigma_wl = 0.0;
        if (!mw_get_stat(sat, &N_wl, &sigma_wl)) continue;
        ...
        if (NFx <= 1) {
            ...
            /* RTKLIB state "f" index is frequency number (0:L1, 1:L2, 2:L5, ...),
               not the observation slot index. */
            const int fidx1 = 0;
            const int fidx5 = 2;
            if (fidx1 >= NFx || fidx5 >= NFx) continue;

            int idx1 = IB(sat, fidx1, &rtk->opt);
            int idx5 = IB(sat, fidx5, &rtk->opt);
